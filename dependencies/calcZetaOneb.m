function [vecSpikeT,vecRealDiff,vecRealFrac,vecRealFracLinear,cellRandT,cellRandDiff,dblZetaP,dblZETA,intZETALoc] = ...
    ...
    [dblMaxD,dblMinD,intZETALoc,intZetaDevIdx_Inv] = getZetaDev(vecRealDiff);
	dblDev = abs(dblMaxD-dblMinD);
    ...
    if boolUseParallel
        parfor intResampling=1:intResampNum
			...
			[dblMaxD,dblMinD] = getZetaDev(vecRandDiff);
			dblRandDev = abs(dblMaxD-dblMinD);
			
			%assign data
			cellRandT{intResampling} = vecThisSpikeTimes;
			cellRandDiff{intResampling} = vecRandDiff;
			vecMaxRandD(intResampling) = dblRandDev;
		end
	else
		for intResampling=1:intResampNum
			%% get random subsample
    ...
    [dblZetaP,dblZETA] = getZetaP(dblDev,vecMaxRandD,boolDirectQuantile);
end
